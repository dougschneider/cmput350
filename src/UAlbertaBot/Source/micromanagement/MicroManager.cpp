...
void MicroManager::execute(const SquadOrder & inputOrder)
{
    ...
    // remove units in or headed to bunkers from units
    std::map<BWAPI::Unit*, std::set<BWAPI::Unit*>>::iterator it = bunkersToUnits.begin(), end = bunkersToUnits.end();
    for(; it != end; ++it)
	{
        std::set<BWAPI::Unit*>::iterator start = it->second.begin();
        for(; start != it->second.end(); ++start)
		{
            std::vector<BWAPI::Unit*>::iterator e = std::find(units.begin(), units.end(), *start);
            if(e != units.end())
			{
                units.erase(e);
			}
			...
			{
                bunkersToUnits.erase(*start);
			}
		}
    }

    // marines should go into bunkers if there are opennings and we're not defending
    if(inputOrder.type == SquadOrder::Attack)
	{
        // find bunkers that are completed and have free space
        BOOST_FOREACH(BWAPI::Unit * u, BWAPI::Broodwar->self()->getUnits())
        {
            if(u->getType() == BWAPI::UnitTypes::Terran_Bunker && !u->isBeingConstructed() &&
                    u->getLoadedUnits().size() < 4)
            {
                if(bunkersToUnits.find(u) != bunkersToUnits.end())
                    continue;
                bunkersToUnits[u] = u->getLoadedUnits();
            }
        }

        // sign units up to be loaded into a bunker
        UnitVector::iterator member = units.begin();
        for(; member != units.end(); ++member)
        {
            if((*member)->getType() == BWAPI::UnitTypes::Terran_Marine)
            {
                std::map<BWAPI::Unit*, std::set<BWAPI::Unit*>>::iterator it = bunkersToUnits.begin(), end = bunkersToUnits.end();
                for(; it != end; ++it)
                {
                    if(4 - it->second.size() > 0)
                    {
                        (*member)->rightClick(it->first);
