...
class StrategyManager 
{
	...
	const bool							expandTerranMnMRush() const;
	const	MetaPairVector		getTerranMarinesAndMedicsBuildOrderGoal() const;
    const   MetaPairVector      getTerranSiegePushBuildOrderGoal() const;
	const	MetaPairVector		getTerranNukesBuildOrderGoal() const;
	const MetaPairVector		getTerranStormDrop() const;
	...
	const bool canResearch(BWAPI::TechType research) const;
	const bool canResearch(BWAPI::UpgradeType research) const;

public:

	enum { ProtossZealotRush=0, ProtossDarkTemplar=1, ProtossDragoons=2, NumProtossStrategies=3 };
	enum { TerranMarineRush=0, TerranMarinesAndMedics=1, TerranSiegePush=2, TerranNukes=3, NumTerranStrategies=4 };
