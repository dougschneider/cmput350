...
void CombatCommander::assignDefenseSquads(std::set<BWAPI::Unit *> & unitsToAssign) 
{
    ...
    BOOST_FOREACH(BWAPI::Unit *u, unitsToAssign)
	{
		...
		{
			siegeTanks.push_back(u);
            unitsToAssign.erase(u);
		}
	}
    BWTA::Chokepoint * chokepoint = *BWTA::getRegion(BWAPI::Broodwar->self()->getStartLocation())->getChokepoints().begin();
	BWAPI::Position baseEntrance = chokepoint->getCenter();
    squadData.addSquad(Squad(siegeTanks, SquadOrder(SquadOrder::Defend, baseEntrance, 300, "Siege Tanks Defend")));
