...
void Squad::update()
{
	...
	if (needToRegroup)
	{
		...
		supportManager.regroup(regroupPosition);
	}
...
void Squad::setManagerUnits()
{
	...
	BOOST_FOREACH(BWAPI::Unit * unit, units)
	{
		if(unit->isCompleted() && unit->getHitPoints() > 0 && unit->exists())
		{
			...
			if(SupportManager::isSupportUnit(unit))
			{
				supportUnits.push_back(unit);
			}
			// select dector units
	...
	supportManager.setUnits(supportUnits);
	supportManager.setAllies(units);
}
