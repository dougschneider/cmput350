...
BWAPI::TilePosition MapTools::getNextExpansion()
{
	...
	{
		// if the base has gas !base->isMineralOnly() && 
		if(!(base == BWTA::getStartLocation(BWAPI::Broodwar->self())))
		{
			...
			int x1 = tile.x() * 32;
			int y1 = tile.y() * 32;
			int x2 = x1 + BWAPI::UnitTypes::Protoss_Nexus.tileWidth() * 32; // same for everyone but should be race specific
			int y2 = y1 + BWAPI::UnitTypes::Protoss_Nexus.tileHeight() * 32;

			// add check from InfoMan about fog of war
			bool fogBuildingInTheWay = false;
			int uimapCount = 0;
			FOR_EACH_UIMAP_CONST(it, InformationManager::Instance().getUnitInfo(BWAPI::Broodwar->enemy()))
			{
				if (it->first->getType().isResourceDepot() || it->second.unit->getType().isResourceDepot())
				{
					uimapCount++;
					// found it, check location, then ignore this if something is there
					/*std::stringstream b; b << "Location: " << 
						base->getTilePosition().x() << " " << base->getTilePosition().y() << " " << 
						it->second.unit->getTilePosition().x() << " " << it->second.unit->getTilePosition().y() <<  
						" IM: " << uimapCount << " B: " << baseCount;
					BWAPI::Broodwar->printf(b.str().c_str());*/
					if (//it->second.lastHealth > 0 &&
							(distance(base->getPosition(), it->second.lastPosition) < 500 ||
							 distance(base->getTilePosition(), it->second.unit->getTilePosition()) < 5) ||
							 distance(base->getTilePosition(), it->first->getTilePosition()) < 5)
					{
						static BWAPI::TilePosition temp;
						if (!temp || temp != base->getPosition())
						{
							temp = base->getTilePosition();
							std::stringstream b; b << "Location taken: " << 
								temp.x() << " " << temp.y() << " " << uimapCount;
							BWAPI::Broodwar->printf(b.str().c_str());
						}
						fogBuildingInTheWay = true;
						break;
					}
					//else { BWAPI::Broodwar->printf("Location acceptable."); }
				}
			}

			if (fogBuildingInTheWay) { continue; }
			...
			if (!BWTA::isConnected(homeTile, tile) || distanceFromHome < 0)	{	continue; }
	...
	{
		static BWAPI::TilePosition temp;
		if (!temp || (closestBase->getPosition() != temp))
		{
			/*temp = closestBase->getTilePosition();
			std::stringstream b; b << "MT Location: " << 
				temp.x() << " " << temp.y();
			BWAPI::Broodwar->printf(b.str().c_str());*/
		}
...
int MapTools::distance(BWAPI::Position a, BWAPI::Position b)
{
	return distance(a.x(), a.y());
}

int MapTools::distance(BWAPI::TilePosition a, BWAPI::TilePosition b)
{
	return distance(a.x(), a.y());
}

int MapTools::distance(int a, int b)
{
	double dx   = a - b;         //horizontal difference 
  double dy   = a - b;         //vertical difference 
  double dist = std::sqrt( dx*dx + dy*dy ); //distance using Pythagora's theorem
  return (int) dist;
}
