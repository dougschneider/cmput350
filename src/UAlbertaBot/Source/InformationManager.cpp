...
void InformationManager::initNeutralBuildings()
{
    std::set<BWAPI::Unit*> units = BWAPI::Broodwar->getNeutralUnits();

    BOOST_FOREACH(BWAPI::Unit* unit, units)
	{
        if(unit->getType().isBuilding())
		{
            neutralBuildings.push_back(UnitInfo(0, unit, unit->getPosition(), unit->getType()));
		}
	}
}

const UnitInfoVector & InformationManager::getNeutralBuildings()
{
