...
class ProductionManager 
{
	...
	BWAPI::Unit *				selectUnitOfType(BWAPI::UnitType type, MetaType producedType, bool leastTrainingTimeRemaining = true, BWAPI::Position closestTo = BWAPI::Position(0,0));
	...
	void						performBuildOrderSearch(const std::vector< std::pair<MetaType, UnitCountType> > & goal, bool clearQueue);
	void						setBuildOrder(const std::vector<MetaType> & buildOrder, bool clearQueue);
	void						createMetaType(BWAPI::Unit * producer, MetaType type);
    bool                        areNearbyNeutralBuildings(BWTA::Chokepoint* point);
	...
	void						cloakedCheck();
	void						defendAgainstCloaked();

	void						autoResearch();
	bool						canResearch(BWAPI::UpgradeType research);
	bool						canResearch(BWAPI::TechType research);
