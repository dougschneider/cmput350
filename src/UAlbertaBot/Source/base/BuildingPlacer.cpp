...
bool BuildingPlacer::canBuildHereWithSpace(BWAPI::TilePosition position, const Building & b, int buildDist, bool horizontalOnly) const
{
	...
	if (b.type.supplyProvided() > 0) height += 1; // prevent trapping workers
    ...
    // leave room for attachments for building to our left
    int searchx = position.x() - 2;
    for(int searchy = position.y(); searchy < position.y() + height; ++searchy)
	{
        BOOST_FOREACH(BWAPI::Unit* unit, BWAPI::Broodwar->getUnitsOnTile(searchx, searchy))
		{
