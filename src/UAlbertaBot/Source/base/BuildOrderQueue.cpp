...
bool BuildOrderQueue::alreadyQueued(MetaType m)
{
	for (std::deque<BuildOrderItem<int>>::iterator it = queue.begin(); it != queue.end(); ++it)
	{
		if ((*it).metaType.getName() == m.getName()) return true;
	}
...
void BuildOrderQueue::removeAll(MetaType m)
{
	std::deque<BuildOrderItem<int>>::iterator it = queue.begin();
	while (it != queue.end())
	{
		if ((*it).metaType.getName() == m.getName())
			it = queue.erase(it);
		else
			++it;
	}
