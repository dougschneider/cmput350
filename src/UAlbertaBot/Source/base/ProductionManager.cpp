	...
	, rushDetected(false)
{
	...
	{
		setBuildOrder(StarcraftBuildOrderSearchManager::Instance().getOpeningBuildOrder(), true);
	}
}

void ProductionManager::setBuildOrder(const std::vector<MetaType> & buildOrder, bool clearQueue)
{
	// clear the current build order
	if (clearQueue)	queue.clearAll();
	
	// for each item in the results build order, add it
	for (size_t i(0); i<buildOrder.size(); ++i)
	{
		// queue the item
		if (buildOrder[i].unitType.isResourceDepot()) // prevent expanding too fast
		{
			if (!BuildMan.isBeingBuilt(BWAPI::UnitTypes::Terran_Command_Center) &&
					BWAPI::Broodwar->self()->minerals() >
					(550 * (1 + (BWAPI::Broodwar->self()->allUnitCount(buildOrder[i].unitType) / 4))))
				/*MetaType b = buildOrder[i];
				std::stringstream a; a << "Building: " << b.getName();
				BWAPI::Broodwar->printf(a.str().c_str());*/
				queue.queueAsLowestPriority(buildOrder[i], true);
			else
				continue;
		}
		else if (buildOrder[i].unitType.isWorker() && WorkerManager::Instance().getNumIdleWorkers() > 14)
		{
			continue;
		}
		else if (buildOrder[i].type == MetaType::Tech)
		{
			if (canResearch(buildOrder[i].techType))
				queue.queueAsLowestPriority(buildOrder[i], true);

		}
		else if (buildOrder[i].type == MetaType::Upgrade)
		{
			if (canResearch(buildOrder[i].upgradeType))
				queue.queueAsLowestPriority(buildOrder[i], true);
		}
		else
		{
...
void ProductionManager::performBuildOrderSearch(const std::vector<std::pair<MetaType, UnitCountType>> & goal, bool clearQueue)
{	
	std::vector<MetaType> buildOrder = StarcraftBuildOrderSearchManager::Instance().findBuildOrder(goal);

	// set the build order
	setBuildOrder(buildOrder, clearQueue);
}
...
void ProductionManager::update() 
{
	...
	{
		performBuildOrderSearch(searchGoal, true);
	}
	...
	if ((queue.size() == 0) && (BWAPI::Broodwar->getFrameCount() > 10) && !Options::Modules::USING_BUILD_ORDER_DEMO)
	{
		...
		std::vector<std::pair<MetaType, UnitCountType>> nonConstNewGoal = StrategyManager::Instance().getBuildOrderGoal();
		std::stringstream a; a << "Num goals: " << newGoal.size();
		int mineralTotal = 0, gasTotal = 0;
		for (std::vector<std::pair<MetaType, UnitCountType>>::iterator it = nonConstNewGoal.begin(); it != nonConstNewGoal.end(); ++it)
		{
			mineralTotal += (*it).first.mineralPrice() * (*it).second;
			gasTotal += (*it).first.gasPrice() * (*it).second;
		}
		a << " Minerals: " << mineralTotal << " Gas: " << gasTotal;
		BWAPI::Broodwar->drawTextScreen(150, 20, a.str().c_str());

		performBuildOrderSearch(newGoal, true);
	}
...
void ProductionManager::autoResearch()
{
	static BWAPI::UpgradeType upgradeCheck = NULL;
	static BWAPI::TechType techCheck = NULL;
	static BWAPI::UnitType unitCheck = NULL;
	// should have a race check
	if (upgradeCheck != NULL || techCheck != NULL || unitCheck != NULL)
	{
		/*if (BWAPI::Broodwar->self()->isResearching(techCheck) || 
			BWAPI::Broodwar->self()->isUpgrading(upgradeCheck) || 
			BuildMan.isBeingBuilt(unitCheck))*/
		performBuildOrderSearch(StrategyManager::Instance().getBuildOrderGoal(), false);
		if (!BWAPI::Broodwar->self()->hasResearched(techCheck)) return;
		else techCheck = NULL;
		if (BWAPI::Broodwar->self()->getUpgradeLevel(upgradeCheck) == 0) return;
		else upgradeCheck = NULL;
		if ((unitCheck == BWAPI::UnitTypes::Terran_Engineering_Bay &&
				 BWAPI::Broodwar->self()->allUnitCount(BWAPI::UnitTypes::Terran_Engineering_Bay) == 0)
				||
				(unitCheck == BWAPI::UnitTypes::Terran_Science_Facility &&
				 BWAPI::Broodwar->self()->allUnitCount(BWAPI::UnitTypes::Terran_Science_Facility) == 0))
			return;
		else unitCheck = NULL;
	}
	int mins = getFreeMinerals();
	if (mins > 1000) // somewhat arbitrary, 2 ccs + change
	{
		if (canResearch(BWAPI::UpgradeTypes::U_238_Shells))
		{ 
			queue.queueAsHighestPriority(MetaType(BWAPI::UpgradeTypes::U_238_Shells), false); 
			upgradeCheck = BWAPI::UpgradeTypes::U_238_Shells;
			return; 
		}
		if (canResearch(BWAPI::TechTypes::Stim_Packs))
		{ 
			queue.queueAsHighestPriority(MetaType(BWAPI::TechTypes::Stim_Packs), false); 
			techCheck = BWAPI::TechTypes::Stim_Packs;
			return; 
		}
		if (canResearch(BWAPI::UpgradeTypes::Caduceus_Reactor))
		{ 
			queue.queueAsHighestPriority(MetaType(BWAPI::UpgradeTypes::Caduceus_Reactor), false); 
			upgradeCheck = BWAPI::UpgradeTypes::Caduceus_Reactor;
			return; 
		}

		if (BWAPI::Broodwar->self()->allUnitCount(BWAPI::UnitTypes::Terran_Engineering_Bay) == 0 &&
				!queue.alreadyQueued(MetaType(BWAPI::UnitTypes::Terran_Engineering_Bay)) &&
				!BuildMan.isBeingBuilt(BWAPI::UnitTypes::Terran_Engineering_Bay))
		{
			queue.queueAsHighestPriority(MetaType(BWAPI::UnitTypes::Terran_Engineering_Bay), false); return;
		}
		else
		{
			if (canResearch(BWAPI::UpgradeTypes::Terran_Infantry_Weapons)) // needs sci facility check
			{ 
				queue.queueAsHighestPriority(MetaType(BWAPI::UpgradeTypes::Terran_Infantry_Weapons), false); 
				upgradeCheck = BWAPI::UpgradeTypes::Terran_Infantry_Weapons;
				return; 
			}
			if (canResearch(BWAPI::UpgradeTypes::Terran_Infantry_Armor))
			{ 
				queue.queueAsHighestPriority(MetaType(BWAPI::UpgradeTypes::Terran_Infantry_Armor), false); 
				upgradeCheck = BWAPI::UpgradeTypes::Terran_Infantry_Armor;
				return; 
			}
		}
	}
	...
	{
		// mid tier, ie factory and maybe starport
	}
	else if (mins > 1800)
	{
		// high tier, ie sci facility
	}
}

bool ProductionManager::canResearch(BWAPI::TechType research)
{
	return !queue.alreadyQueued(MetaType(research)) &&
				 !BWAPI::Broodwar->self()->isResearching(research) &&
				 !BWAPI::Broodwar->self()->hasResearched(research);
}

bool ProductionManager::canResearch(BWAPI::UpgradeType research)
{
	return !queue.alreadyQueued(MetaType(research)) &&
				 !BWAPI::Broodwar->self()->isUpgrading(research) &&
				 (BWAPI::Broodwar->self()->getUpgradeLevel(research) < 1);
					//BWAPI::Broodwar->self()->getMaxUpgradeLevel(research));
}

void ProductionManager::cloakedCheck()
{
		// if they have cloaked units get a new goal asap
	if (!enemyCloakedDetected && InformationManager::Instance().enemyHasCloakedUnits())
	{
		defendAgainstCloaked();

		BWAPI::Broodwar->printf("Enemy Cloaked Unit Detected!");
		enemyCloakedDetected = true;
	}
	else if (enemyCloakedDetected && InformationManager::Instance().enemyHasCloakedUnits())
	{
		//defendAgainstCloaked();
		if (BWAPI::Broodwar->self()->allUnitCount(BWAPI::UnitTypes::Terran_Missile_Turret) < 1 &&
				!queue.alreadyQueued(MetaType(BWAPI::UnitTypes::Terran_Missile_Turret)) &&
				!BuildMan.isBeingBuilt(BWAPI::UnitTypes::Terran_Missile_Turret))
		{
			queue.queueAsHighestPriority(MetaType(BWAPI::UnitTypes::Terran_Marine), false);
			queue.queueAsHighestPriority(MetaType(BWAPI::UnitTypes::Terran_Marine), false);
			queue.queueAsHighestPriority(MetaType(BWAPI::UnitTypes::Terran_Marine), false);
...
void ProductionManager::defendAgainstCloaked()
{
	...
	else if (BWAPI::Broodwar->self()->getRace() == BWAPI::Races::Terran)
	{
		if (BWAPI::Broodwar->self()->allUnitCount(BWAPI::UnitTypes::Terran_Bunker) < 1 &&
				!BuildMan.isBeingBuilt(BWAPI::UnitTypes::Terran_Bunker))
		{
		...
		{
			queue.queueAsHighestPriority(MetaType(BWAPI::UnitTypes::Terran_Comsat_Station), true);
			queue.queueAsHighestPriority(MetaType(BWAPI::UnitTypes::Terran_Missile_Turret), true);
			queue.queueAsHighestPriority(MetaType(BWAPI::UnitTypes::Terran_Missile_Turret), true);
			queue.queueAsHighestPriority(MetaType(BWAPI::UnitTypes::Terran_Missile_Turret), true);
			queue.queueAsLowestPriority(MetaType(BWAPI::UnitTypes::Terran_Marine), false);
			queue.queueAsLowestPriority(MetaType(BWAPI::UnitTypes::Terran_Marine), false);
			queue.queueAsLowestPriority(MetaType(BWAPI::UnitTypes::Terran_Marine), false);
			queue.queueAsLowestPriority(MetaType(BWAPI::UnitTypes::Terran_Marine), false);
			queue.queueAsLowestPriority(MetaType(BWAPI::UnitTypes::Terran_Marine), false);
      ...
      if (unit->getType() != BWAPI::UnitTypes::Zerg_Drone)
      {
        if (WorkerManager::Instance().isBuilder(unit))
        {
           BWAPI::Broodwar->printf("Critical unit died, re-searching build order");
					performBuildOrderSearch(StrategyManager::Instance().getBuildOrderGoal(), true);
        }
      }
		}
		else if (unit->getType() == BWAPI::UnitTypes::Terran_Academy &&
			BWAPI::Broodwar->self()->allUnitCount(BWAPI::UnitTypes::Terran_Academy) < 1)
		{
			if (queue.alreadyQueued(MetaType(BWAPI::UnitTypes::Terran_Firebat)))
				queue.removeAll(MetaType(BWAPI::UnitTypes::Terran_Firebat));
			if (queue.alreadyQueued(MetaType(BWAPI::UnitTypes::Terran_Medic)))
				queue.removeAll(MetaType(BWAPI::UnitTypes::Terran_Medic));
		}
	}
}
...
void ProductionManager::manageBuildOrderQueue() 
{
	...
	while (!queue.isEmpty()) 
	{
		...
		if (producer && canMake) // if we can make the current item
		{
			// create it
		...
		else if (queue.canSkipItem()) // otherwise, if we can skip the current item
		{
			// skip it
			queue.skipItem();
...
void ProductionManager::createMetaType(BWAPI::Unit * producer, MetaType t) 
{
		...
		&& t.unitType != BWAPI::UnitTypes::Zerg_Greater_Spire)
	{
        ...
        if(t.unitType == BWAPI::UnitTypes::Terran_Bunker ||
                t.unitType == BWAPI::UnitTypes::Terran_Missile_Turret)
        {
              BWAPI::TilePosition start = BWAPI::Broodwar->self()->getStartLocation();
              std::set<BWTA::Chokepoint*>::const_iterator entrance = BWTA::getRegion(start)->getChokepoints().begin(),
				                                          end      = BWTA::getRegion(start)->getChokepoints().end();
              for(; entrance != end; ++entrance)
			  {
                  if(location == BWAPI::Broodwar->self()->getStartLocation() || !areNearbyNeutralBuildings(*entrance))
				  {
                      double firstSideDist = abs((*entrance)->getSides().first.getDistance(BWAPI::Position(start)));
                      double secondSideDist = abs((*entrance)->getSides().second.getDistance(BWAPI::Position(start)));
                      if(firstSideDist > secondSideDist)
                      {
                          location = BWAPI::TilePosition((*entrance)->getSides().first);
                      }
                      else
                      {
                          location = BWAPI::TilePosition((*entrance)->getSides().second);
                      }
				  }
			  }
        }
		// send the building task to the building manager
		BuildingManager::Instance().addBuildingTask(t.unitType, location);
	}
...
bool ProductionManager::areNearbyNeutralBuildings(BWTA::Chokepoint* point)
{
    const UnitInfoVector nearbyUnits = InformationManager::Instance().getNeutralBuildings();
    BOOST_FOREACH(UnitInfo unit, nearbyUnits)
	{
        if(point->getCenter().getDistance(unit.lastPosition) < 200)
            return true;
	}
    return false;
}
...
BWAPI::Unit * ProductionManager::selectUnitOfType(BWAPI::UnitType type, MetaType producedType, bool leastTrainingTimeRemaining, BWAPI::Position closestTo)
{
	...
	if (closestTo != BWAPI::Position(0,0)) 
	{
		...
		{
			if (u->getType() == type && (!producedType.requiresAddon() || u->getAddon() != NULL))
			{
	...
	else if (type.isBuilding() && leastTrainingTimeRemaining)
	{
		BOOST_FOREACH (BWAPI::Unit * u, BWAPI::Broodwar->self()->getUnits())
		{
			if (u->getType() == type && u->isCompleted() && !u->isTraining() && !u->isLifted() &&!u->isUnpowered() &&
				    (!producedType.requiresAddon() || u->getAddon() != NULL))
			{
				return u;
			}
		}
	} 
	else // otherwise just return the first unit we come across
	{
		BOOST_FOREACH(BWAPI::Unit * u, BWAPI::Broodwar->self()->getUnits()) 
		{
