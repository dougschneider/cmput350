...
void BuildingManager::assignWorkersToUnassignedBuildings() 
{
	...
	while (buildingData.hasNextBuilding(ConstructionData::Unassigned)) 
	{
		...
		if (b.isAddon())
		{
			BWAPI::UnitType required = b.getAddonBuilding();
			BOOST_FOREACH(BWAPI::Unit *building, BWAPI::Broodwar->self()->getUnits())
			{
				BWAPI::TilePosition basePos = building->getTilePosition();
				BWAPI::TilePosition addonPos = BWAPI::TilePosition(basePos.x() + building->getType().tileWidth(),
																   basePos.y() + building->getType().tileHeight() - 2);
				if(required.getID() == building->getType().getID())
				{
					BuildingPlacer::Instance().reserveTiles(addonPos, b.type.tileWidth(), b.type.tileHeight());
					b.builderUnit = building;
					b.finalPosition = addonPos;
					buildingData.removeCurrentBuilding(ConstructionData::Unassigned);
					buildingData.addBuilding(ConstructionData::Assigned, b);
					// once we find a usable building to add this addon to, we're done
					break;
				}
			}
			// we're done with the building now
			continue;
		}
...
BWAPI::TilePosition BuildingManager::getBuildingLocation(const Building & b)
{
	...
	{
		testLocation = BuildingPlacer::Instance().getBuildLocationNear(b, 0, false);
	}
	...
	else
	{
		...
		else if (b.type.isResourceDepot())
		{
			...
			if (tile != BWAPI::TilePositions::None)
			{
				...
				{
					/*temp = tile;
					std::stringstream b; b << "Same as MT Location: " << 
						temp.x() << " " << temp.y();
		...
		else
		{
			...
			if (!b.builderUnit->isConstructing()) 
			{
				...
				else
				{
					...
					{
						static BWAPI::TilePosition temp;
						if (!temp || b.finalPosition != temp)
						{
							temp = b.finalPosition;
							std::stringstream a; a << "CC BM Location: " << 
								temp.x() << " " << temp.y();
...
void BuildingManager::checkForDeadTerranBuilders() 
{
	...
	while (buildingData.hasNextBuilding(ConstructionData::UnderConstruction))
	{
		...
		if (!b.buildingUnit->isBeingConstructed() && !b.buildingUnit->isCompleted())
		{
            b.builderUnit = NULL;
			BWAPI::Unit* freeWorker = WorkerManager::Instance().getBuilder(b);
            if(freeWorker)
			{
                b.builderUnit = freeWorker;
...
void BuildingManager::checkForCompletedBuildings() 
{
	...
	while (buildingData.hasNextBuilding(ConstructionData::UnderConstruction)) 
	{
		...
		if (b.buildingUnit->isCompleted()) 
		{
			...
			if (BWAPI::Broodwar->self()->getRace() == BWAPI::Races::Terran && !b.isAddon())
			{
				WorkerManager::Instance().finishedWithWorker(b.builderUnit);
			}
